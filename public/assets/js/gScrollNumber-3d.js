...
var r_index = 0;
function gScrollNumber3D(el,option) { // 滾輪初始化
    this.container = $(el);
    this.option = option;
    this.container.css({
        position: "relative",
    ...
    this.col2 = '<span class="g-num-item" id="g-num-item" href="0">' +
        '<i class="g-wheel-item g-wheel-item-0"><a>.</a></i>' +
        '<i class="g-wheel-item g-wheel-item-1"><a>.</a></i>' +
        '<i class="g-wheel-item g-wheel-item-2"><a>.</a></i>' +
        '<i class="g-wheel-item g-wheel-item-3"><a>.</a></i>' +
        '<i class="g-wheel-item g-wheel-item-4"><a>.</a></i>' +
        '<i class="g-wheel-item g-wheel-item-5"><a>.</a></i>' +
        '<i class="g-wheel-item g-wheel-item-6"><a>.</a></i>' +
        '<i class="g-wheel-item g-wheel-item-7"><a>.</a></i>' +
        '<i class="g-wheel-item g-wheel-item-8"><a>.</a></i>' +
        '<i class="g-wheel-item g-wheel-item-9"><a>.</a></i>' +
        '</span>';
}

gScrollNumber3D.prototype.run = function (value) { // 執行滾動動作

    function toDecimal(x) { // 傳入值轉Float，保持小數點後兩位
        var f = parseFloat(x); 
        if (isNaN(f)) {
            return false;
        }
    ...
    var h = (self.subHeight / Math.tan((angle * 0.5) * (2 * Math.PI/360))) + 2;
    ...
    if (valueString.length > oldLength) {
        for (var i = 0; i < valueString.length - oldLength; i++) {
            if(i == 2) {
                self.container.append(self.col2);
            } else {
